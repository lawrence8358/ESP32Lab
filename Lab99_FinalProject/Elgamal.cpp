...
// 加密函式
uint64_t* Elgamal::Encrypt(uint32_t message, uint64_t Y, uint64_t x)
{ 
  uint64_t *ret = (uint64_t*)malloc(sizeof(uint64_t) * 2);  // 需要free(ret)
  uint64_t K = _powof(Y, x, _params.p);
  uint64_t c = (K * message) % (_params.p);
...
// 解密函式 Decryption:D(c)= m = K * c^(-1) mod p
uint64_t Elgamal::Decrypt(uint64_t cipher, uint64_t K)
{  
  return (cipher * _mod_inverse(K, _params.p)) % _params.p;
}  

uint64_t Elgamal::GenY(uint64_t y)
{ 
  return _powof(_params.g, y, _params.p);
}
