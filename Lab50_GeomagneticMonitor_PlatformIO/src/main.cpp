...
/**
 * Lab50_GeomagneticMonitor_PlatformIO - 地磁監測器
 * 
 * 功能說明：
...
// BMM150 讀取間隔 (毫秒) - 快速更新以達到指北針效果
const uint32_t BMM150_READ_INTERVAL_MS = 200;
// MQTT 發送間隔 (毫秒) - 60 秒發送一次
const uint32_t MQTT_SEND_INTERVAL_MS = 60000;
...
// WiFi 資訊緩存（用於 OLED 顯示）
String currentLine1 = "Initializing...";

// ============== WiFi 資訊更新回調 ==============
void onWiFiInfoUpdate()
{
    currentLine1 = "Mode: " + WiFiCtrl.getMode() + "\nSSID: " + WiFiCtrl.getSSID() + "\nIP: " + WiFiCtrl.getIP();
}
            ...
            if (MQTT.connected())
            {
                ...
                if (xSemaphoreTake(magDataMutex, portMAX_DELAY) == pdTRUE)
                {
                    if (magDataBuffer.length() > 0)
                    {
                        ...
                        String jsonPayload = "{";
                        jsonPayload += "\"x\":" + String(data.xx, 2) + ",";
                        jsonPayload += "\"y\":" + String(data.yy, 2) + ",";
                        jsonPayload += "\"z\":" + String(data.zz, 2) + ",";
...
void BMM150Thread(void *pvParameters) 
{
    ...
    for (;;)
    {
        ...
        // 更新 OLED 顯示（含圖形化指北針）
        OLCD.printGeomagneticData(currentLine1, data.xx, data.yy, data.zz, degree);
...
void setup()
{
    ...
    Serial.println("=================================");
    Serial.println("Lab50_GeomagneticMonitor Starting...");
    Serial.println("=================================");
